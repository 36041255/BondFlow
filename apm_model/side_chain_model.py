...
from apm_model.node_feature_net import FullNodeFeatureNet, PackingNodeFeatureNet
from apm_model.edge_feature_net import FullEdgeFeatureNet
from apm_model import ipa_pytorch
from multiflow_data import utils as du
import numpy as np
from multiflow_data import so3_utils
...
class AngleResnet(nn.Module):
    ...
    def __init__(self, c_in, c_hidden, no_blocks, no_angles, epsilon, use_initial=True):
        """
        Args:
            c_in:
    ...
    def forward(
        self, s: torch.Tensor, s_initial: torch.Tensor=None
    ):
        """
        Args:
            s:
        ...
        s = self.relu(s)
        s = self.linear_in(s)
        if s_initial is not None and self.use_initial:
...
class SideChainModel(nn.Module):
    ...
    def forward(self, input_feats):
        ...
        if 'trans_1' in input_feats:
            ...
            trans_for_sidechain = trans_t_pred_1

        if 'rotmats_1' in input_feats:
            ...
            rotmats_for_sidechain = rotmats_t_pred_1

        if 'aatypes_1' in input_feats:
            ...
            aatypes_for_sidechain = aatypes_t_pred_1
        ...
        if self.flow_based:
            ...
            # without flow, torsions_t is not used
            torsions_t = torsions_t * 2 * np.pi * 0 # [0-2pi]
        ...
        # Add external node embed if provided.
        if 'external_node_embed' in input_feats and input_feats['external_node_embed'] is not None:
            init_node_embed = init_node_embed + input_feats['external_node_embed']
        ...
        # Add external pair embed if provided.
        if 'external_pair_embed' in input_feats and input_feats['external_pair_embed'] is not None:
            init_edge_embed = init_edge_embed + input_feats['external_pair_embed']
