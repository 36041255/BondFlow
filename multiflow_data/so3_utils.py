...
def skew_matrix_exponential_map(
    ...
    # Get standard terms.
    sin_coeff = torch.sin(angles) / (angles+ 1e-8)
    cos_coeff = (1.0 - torch.cos(angles)) / (angles_sq + 1e-8)
    ...
    def _setup_lookup(
        ...
        if cache_dir is not None:
            ...
            if cache.path_exists and not overwrite_cache:
                print( f"Cache {cache.cache_path} exists. Loading data from cache.")
                # Load data from cache.
                cache_data = cache.load_cache()
                omega_grid = cache_data["omega_grid"]
                cdf_igso3 = cache_data["cdf_igso3"]
            else:
                print(f"Cache {cache.cache_path} does not exist or overwrite requested. Generating lookup tables.")
