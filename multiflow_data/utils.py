...
from Bio import PDB
from multiflow_data import protein, residue_constants, parsers
...
def _get_torch_scatter_ops():
    """Lazily import torch_scatter to avoid import-time hard dependency."""
    try:
        from torch_scatter import scatter_add, scatter
    except Exception as e:
        raise ImportError(
            "torch_scatter is required for structure alignment utilities "
            "(center_zero/align_structures). Please install a torch_scatter "
            "build matching your PyTorch/CUDA version."
        ) from e
    return scatter_add, scatter
...
def center_zero(pos: torch.Tensor, batch_indexes: torch.LongTensor) -> torch.Tensor:
    ...
    _, scatter = _get_torch_scatter_ops()
...
def align_structures(
    ...
    scatter_add, _ = _get_torch_scatter_ops()
    cov = scatter_add(
