...
def writepdb(
    ...
    def _infer_element(atom_name):
        """Infer element symbol for PDB columns 77-78 from atom name."""
    ...
    def _format_atom_line(serial, atom_name, res_name, chain_id, resseq, xyz, occ, bfact):
        atom4 = f"{str(atom_name):>4}"[:4]
        element = _infer_element(atom_name)
        # PDB fixed-width columns with element in cols 77-78.
        return (
            f"ATOM  {int(serial):5d} {atom4} {str(res_name):>3} {str(chain_id):1}"
            f"{int(resseq):4d}    {float(xyz[0]):8.3f}{float(xyz[1]):8.3f}{float(xyz[2]):8.3f}"
            f"{float(occ):6.2f}{float(bfact):6.2f}          {element:>2}\n"
        )
    ...
    for i, s in enumerate(scpu):
        ...
        if len(atomscpu.shape) == 2:
            if torch.isnan(atomscpu[i, :]).any() or (atomscpu[i, :].abs() < 1e-7).all():
                continue
            f.write(_format_atom_line(ctr, " CA ", num2aa[s], chain, idx_pdb[i], atomscpu[i], 1.0, Bfacts[i]))
            atom_serial_map[(i, 0)] = ctr
            ctr += 1
        elif atomscpu.shape[1] == 3:
            for j, atm_j in enumerate([" N  ", " CA ", " C  "]):
                if torch.isnan(atomscpu[i, j, :]).any() or (atomscpu[i, j, :].abs() < 1e-7).all():
                    continue
                f.write(_format_atom_line(ctr, atm_j, num2aa[s], chain, idx_pdb[i], atomscpu[i, j], 1.0, Bfacts[i]))
            ...
            for j, atm_j in enumerate(atms):
                if (
                    ...
                    and not (atomscpu[i, j, :].abs() < 1e-7).all()
                ):
                    f.write(_format_atom_line(ctr, atm_j, num2aa[s], chain, idx_pdb[i], atomscpu[i, j], 1.0, Bfacts[i]))
                    atom_serial_map[(i, j)] = ctr
                    ctr += 1
        ...
        for i, s in enumerate(scpu):
            ...
            for j, atm_j in enumerate(atms):
                ...
                if not use_hydrogens and j >= N_HEAVY:
                    break
                if (atm_j is None) or (torch.isnan(atomscpu[i, j]).any()) or (atomscpu[i, j].abs() < 1e-7).all():
                    continue
                chain_id = "A"
                if chain_ids is not None:
                    chain_id = chain_ids[i]
                f.write(_format_atom_line(ctr, atm_j, num2aa[s], chain_id, i + 1, atomscpu[i, j], 1.0, Bfacts[i]))
                atom_serial_map[(i, j)] = ctr
                ctr += 1
