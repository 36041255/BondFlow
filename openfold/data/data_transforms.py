...
def atom37_to_frames(protein, eps=1e-8):
    ...
    batch_dims = len(aatype.shape[:-1]) # 1

    restype_rigidgroup_base_atom_names = np.full([21, 8, 3], "", dtype=object) # 21 AAs, 8 groups, 3 atoms per group
    restype_rigidgroup_base_atom_names[:, 0, :] = ["C", "CA", "N"] # backbone frame
    restype_rigidgroup_base_atom_names[:, 3, :] = ["CA", "C", "O"] # O frame
        ...
        for chi_idx in range(4):
            if rc.chi_angles_mask[restype][chi_idx]:
                ...
                restype_rigidgroup_base_atom_names[
                    restype, chi_idx + 4, :
                ] = names[1:] # the last four groups refer to the sidechain groups
    ...
    restype_rigidgroup_mask[..., :20, 4:] = all_atom_mask.new_tensor(
        rc.chi_angles_mask
    ) # [B, N_res, 21, 8], 1 refers to the existence of the corresponding group
    ...
    gt_frames = Rigid.from_3_points(
        p_neg_x_axis=base_atom_pos[..., 0, :], # [B, N_res, 21, 8, 3], 1st atom coords in the frame
        origin=base_atom_pos[..., 1, :], # [B, N_res, 21, 8, 3], 2nd atom coords in the frame
        p_xy_plane=base_atom_pos[..., 2, :], # [B, N_res, 21, 8, 3], 3rd atom coords in the frame
        eps=eps,
    )
...
def atom37_to_torsion_angles(
    ...
    all_atom_positions = protein[prefix + "all_atom_positions"]  # assume shape as [B, N_res, 37, 3]
    all_atom_mask = protein[prefix + "all_atom_mask"] # assume shape as [B, N_res, 37]

    # Upcast to float64 for improved numerical precision in torsion computation
    orig_dtype = all_atom_positions.dtype
    all_atom_positions = all_atom_positions.to(torch.float64)
    all_atom_mask = all_atom_mask.to(torch.float64)

    aatype = torch.clamp(aatype, max=20)

    pad = all_atom_positions.new_zeros(
        [*all_atom_positions.shape[:-3], 1, 37, 3] # [B, 1, 37, 3]
    )
    prev_all_atom_positions = torch.cat(
        [pad, all_atom_positions[..., :-1, :, :]], dim=-3 # previous residue coordinates, [B, N_res, 37, 3], pad + Res[:-1]
    )

    pad = all_atom_mask.new_zeros([*all_atom_mask.shape[:-2], 1, 37]) # [B, 1, 37]
    prev_all_atom_mask = torch.cat([pad, all_atom_mask[..., :-1, :]], dim=-2) # [B, N_res, 37]

    pre_omega_atom_pos = torch.cat(
        [prev_all_atom_positions[..., 1:3, :], all_atom_positions[..., :2, :]],
        dim=-2,
    ) # prev_Res [CA, C] + curr_Res [N, CA], torsion of C_prev - N_curr, peptide bond
    phi_atom_pos = torch.cat(
        [prev_all_atom_positions[..., 2:3, :], all_atom_positions[..., :3, :]],
        dim=-2,
    ) # pre_Res [C, ] + curr_Res [N, CA, C], torsion of N_curr - CA_curr
    psi_atom_pos = torch.cat(
        [all_atom_positions[..., :3, :], all_atom_positions[..., 4:5, :]],
        dim=-2,
    ) #  pre_Res [N, CA, C] + curr_Res [O, ], torsion of CA_curr - C_curr
    # shape as [B, N_res, 4, 3]
    ...
    torsions_atom_pos = torch.cat(
        ...
        dim=-3,
    ) # shape as [B, N_res, 7, 4, 3], 7 -> 7 torsions, 4 -> 4 atoms define a torsion, 3 -> 3D coordinates
    ...
    torsion_frames = Rigid.from_3_points(
        ...
        torsions_atom_pos[..., 0, :],
        eps=1e-12,
    ) # the frame made by first-3 atoms of the 4-torsion atoms

    fourth_atom_rel_pos = torsion_frames.invert().apply(
        torsions_atom_pos[..., 3, :]
    ) # the invert transformation of the current rigid applied to the 4th atom
    ...
    # Cast results back to original dtype to maintain downstream expectations
    protein[prefix + "torsion_angles_sin_cos"] = torsion_angles_sin_cos.to(orig_dtype)
    protein[prefix + "alt_torsion_angles_sin_cos"] = alt_torsion_angles_sin_cos.to(orig_dtype)
    protein[prefix + "torsion_angles_mask"] = torsion_angles_mask.to(orig_dtype)
