...
def run_sampling_worker(device_str, cfg_path, num_designs, num_cycle, num_timesteps, 
    ...
    # Multi-process safety: avoid identical RNG streams across workers when seed is fixed.
    # Keep runs reproducible by deriving a deterministic per-worker seed offset.
    base_seed = getattr(cfg.inference, "seed", None) if hasattr(cfg, "inference") else None
    if base_seed is not None:
        worker_offset = int(batch_range[0])
        if device.type == "cuda":
            worker_offset += int(gpu_id) * 1000
        cfg.inference.seed = int(base_seed) + worker_offset
        print(f"[{device_label}] Seed override: base={base_seed}, offset={worker_offset}, worker_seed={cfg.inference.seed}")
    ...
    if use_partial_diffusion:
        ...
        if getattr(cfg.design_config, "contigs", None) is not None:
            ...
            print("head_mask",head_mask)
            print("tail_mask",tail_mask)
            # 打印完整的 bond_mask 和 ss_init
            torch.set_printoptions(profile="full")
            print("bond_mask", bond_mask[0])
            print("ss_init", ss_init[0])
            ...
            for local_batch_i in range(num_batches_this_gpu):
                ...
                sampler.sample_from_prior(
                    ...
                    hotspots=hotspots.repeat(num_designs, 1),
                    start_index=int(start_index),
                    pdb_parsed=pdb_parsed if getattr(cfg.design_config, 'input_pdb', None) else None,  # 传递原始PDB数据，用于写入完整链
                )
        else:
