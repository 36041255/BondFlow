...
def compute_energy_for_pdb(
    ...
    try:
        ...
        # 记录自动头尾建键成功的候选对（用于后续决定是否写可视化 LINK）
        head_tail_bonded_pairs = set()
            ...
            for (pC, aC, pN, aN, rC, rN, ch, seqC, seqN) in _iter_head_tail_candidates_from_pose(pose):
                try:
                    ...
                    if not pose.residue(int(pC)).is_bonded(pose.residue(int(pN))):
                        ...
                        head_tail_bonded_pairs.add((int(pC), int(pN)))
                except Exception as e:
        ...
        if save_relaxed_pdb:
            ...
            try:
                ...
                # If user requested head-tail constraint/bond, optionally add synthetic LINK lines for visualization.
                # Only append when geometry is plausible or the bond is actually present in the pose.
                # This avoids writing misleading LINK records for clearly failed head-tail cyclization attempts.
                if auto_head_tail_constraint or auto_head_tail_bond:
                    try:
                        # append head-tail LINKs (best-effort)
                        head_tail_link_max_distance = 2.5  # Angstrom
                        ht_lines = []
                        for (pC, aC, pN, aN, rC, rN, ch, seqC, seqN) in _iter_head_tail_candidates_from_pose(pose):
                            pC_i = int(pC)
                            pN_i = int(pN)
                            d = float((pose.residue(pC_i).xyz("C") - pose.residue(pN_i).xyz("N")).norm())
                            is_bonded = False
                            try:
                                is_bonded = bool(pose.residue(pC_i).is_bonded(pose.residue(pN_i)))
                            except Exception:
                                is_bonded = False

                            geometry_ok = (d <= head_tail_link_max_distance)
                            auto_bond_succeeded = ((pC_i, pN_i) in head_tail_bonded_pairs)
                            should_append = is_bonded or auto_bond_succeeded or geometry_ok
