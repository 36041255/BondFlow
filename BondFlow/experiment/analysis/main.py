...
from analyze_ligand_aa_diff import compare_ligand_aa_distribution
from analyze_ligand_ss_diff import compare_ligand_ss_distribution
...
def main():
    ...
    parser.add_argument("--tmalign_chain1", type=str, default=None, help="TMalign中作为structure_1使用的链ID（如A）")
    parser.add_argument("--tmalign_chain2", type=str, default=None, help="TMalign中作为structure_2使用的链ID（如B）")
    ...
    # ======================== Ligand 氨基酸分布差异 ==========
    parser.add_argument("--useLigandAADiff", action="store_true", help="是否比较设计 ligand 与 benchmark ligand 的氨基酸分布差异")
    parser.add_argument("--benchmark_input", type=str, default=None, help="benchmark 结构目录（用于 ligand 分布对比）")
    parser.add_argument("--ligand_chain", type=str, default="A", help="ligand 链 ID（默认 A）")
    parser.add_argument("--save_aadiff_results", action="store_true", help="是否保存 ligand 分布差异结果")
    # ======================== Ligand 二级结构分布差异 ==========
    parser.add_argument("--useLigandSSDiff", action="store_true", help="是否比较设计 ligand 与 benchmark ligand 的二级结构分布差异")
    parser.add_argument("--benchmark_ss_input", type=str, default=None, help="benchmark 结构目录（用于 ligand 二级结构分布对比，默认复用 benchmark_input）")
    parser.add_argument("--save_ssdiff_results", action="store_true", help="是否保存 ligand 二级结构分布差异结果")
    parser.add_argument("--ss_no_dssp", action="store_true", help="二级结构统计时禁用 DSSP，改用 phi/psi 近似分类")
    # ======================== 计算位点香农熵 ==========
    ...
    if args.useTMalign:
        ...
        df_tmalign, rmsd_matrix, tm_matrix = batch_tmalign(
            ...
            chain1_id=args.tmalign_chain1,
            chain2_id=args.tmalign_chain2,
        )
    ...
    # ==================== Ligand AA Distribution Difference ====================
    df_aadiff = None
    aadiff_summary = None
    if args.useLigandAADiff:
        if not args.benchmark_input:
            print("ERROR: 使用 --useLigandAADiff 时必须提供 --benchmark_input")
            sys.exit(1)
        benchmark_path = Path(args.benchmark_input)
        if not benchmark_path.exists() or not benchmark_path.is_dir():
            print(f"ERROR: benchmark 目录不存在或不是目录: {benchmark_path}")
            sys.exit(1)

        print(
            f"Start Ligand AA Distribution Diff: design={input_path}, benchmark={benchmark_path}, chain={args.ligand_chain}"
        )
        df_aadiff, aadiff_summary = compare_ligand_aa_distribution(
            design_dir=str(input_path),
            benchmark_dir=str(benchmark_path),
            chain_id=args.ligand_chain,
            output_dir=str(output_path),
            save_results=args.save_aadiff_results,
        )
        if aadiff_summary is not None:
            print(
                f"Ligand AA 差异完成: L1={aadiff_summary['L1_Distance']:.4f}, "
                f"JS={aadiff_summary['JS_Divergence']:.4f}"
            )

    # ==================== Ligand SS Distribution Difference ====================
    df_ssdiff = None
    ssdiff_summary = None
    if args.useLigandSSDiff:
        benchmark_ss_dir = args.benchmark_ss_input if args.benchmark_ss_input else args.benchmark_input
        if not benchmark_ss_dir:
            print("ERROR: 使用 --useLigandSSDiff 时必须提供 --benchmark_ss_input 或 --benchmark_input")
            sys.exit(1)

        benchmark_ss_path = Path(benchmark_ss_dir)
        if not benchmark_ss_path.exists() or not benchmark_ss_path.is_dir():
            print(f"ERROR: 二级结构 benchmark 目录不存在或不是目录: {benchmark_ss_path}")
            sys.exit(1)

        print(
            f"Start Ligand SS Distribution Diff: design={input_path}, benchmark={benchmark_ss_path}, chain={args.ligand_chain}"
        )
        df_ssdiff, ssdiff_summary = compare_ligand_ss_distribution(
            design_dir=str(input_path),
            benchmark_dir=str(benchmark_ss_path),
            chain_id=args.ligand_chain,
            output_dir=str(output_path),
            save_results=args.save_ssdiff_results,
            prefer_dssp=(not args.ss_no_dssp),
        )
        if ssdiff_summary is not None:
            print(
                f"Ligand SS 差异完成: L1={ssdiff_summary['L1_Distance']:.4f}, "
                f"JS={ssdiff_summary['JS_Divergence']:.4f}"
            )
