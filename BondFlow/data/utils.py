...
import torch
from Bio.PDB import MMCIFParser, MMCIF2Dict, PDBParser
...
def parse_pdb(filename, **kwargs):
    """
    Extract xyz coords for all heavy atoms from a pdb file using BioPython.
    """
    parser = PDBParser(QUIET=True)
    structure = parser.get_structure("pdb_structure", filename)
    with open(filename, "r") as f:
        pdb_lines = f.readlines()
    return parse_pdb_structure(structure, pdb_lines, **kwargs)


def parse_pdb_structure(structure, pdb_lines, parse_hetatom=False, ignore_het_h=True,
                        parse_link=True, link_csv_path=None):
    """
    Extracts information from a BioPython PDB structure object, mirroring parse_cif_structure.
    Output keys and shapes are aligned with parse_cif_structure.
    """
    res, pdb_idx, rf_idx = [], [], []
    rf_index = 0
    last_index = None
    ...
    if parse_link:
        ...
        # Parse LINK / SSBOND records from raw PDB lines.
        for line in pdb_lines:
            rec = line[:6].strip().upper()
            if rec not in ("LINK", "SSBOND"):
                continue
            try:
                if rec == "SSBOND":
                    p1_res_name = line[11:14].strip()
                    p1_chain = line[15:16].strip()
                    p1_res_num = line[17:21].strip()
                    p1_ins_code = line[21:22].strip()
                    p1_atom_name = "SG"

                    p2_res_name = line[25:28].strip()
                    p2_chain = line[29:30].strip()
                    p2_res_num = line[31:35].strip()
                    p2_ins_code = line[35:36].strip()
                    p2_atom_name = "SG"
                else:
                    p1_atom_name = line[12:16].strip()
                    p1_res_name = line[17:20].strip()
                    p1_chain = line[21:22].strip()
                    p1_res_num = line[22:26].strip()
                    p1_ins_code = line[26:27].strip()

                    p2_atom_name = line[42:46].strip()
                    p2_res_name = line[47:50].strip()
                    p2_chain = line[51:52].strip()
                    p2_res_num = line[52:56].strip()
                    p2_ins_code = line[56:57].strip()
                ...
                if idx1 == idx2:
                    continue
                if not (idx1 in pdb_idx and idx2 in pdb_idx):
                    continue
                ...
                if atom1_obj is None or atom2_obj is None:
                    continue
                ...
                print(f"Could not parse a LINK/SSBOND record from PDB: {e}")
                continue
...
def process_target(
    ...
    if ext in [".cif", ".mmcif"]:
        target_struct = parse_cif(
            pdb_path,
            parse_hetatom=parse_hetatom,
        ...
        raise ValueError(
            f"Unsupported structure format for {pdb_path}. "
            f"Expected .pdb/.ent or .cif/.mmcif"
        )
...
class Target:
    ...
    def __init__(self, conf: DictConfig, pdb_parsed, N_C_add=True, nc_pos_prob = 0.3, inference: bool = False):
        ...
        if getattr(self, "inference", False):
            ...
            # [ADD] 辅助：创建一个集合，包含所有在 contigs 中明确请求的原始残基
            # 这样我们可以区分“用户想要的固定残基”和“自动补全的背景残基”
            requested_origins = set(self.full_origin_pdb_idx)

            # [ADD] 辅助：判断某条链是否包含 New_ 元素（即是否为设计链）
            # 我们检查该链是否有对应的 pseudo origins
            chain_has_new = {}
            for ch in contig_chain_order:
                 chain_has_new[ch] = len(_pseudo_origins_for_chain(ch)) > 0
            ...
            for ch in contig_chain_order:
                ...
                if ch in orig_chains:
                    ...
                    # [MODIFIED] 核心修复逻辑
                    is_design_chain = chain_has_new.get(ch, False)

                    for o in orig_idx_full:
                        if o[0] != ch:
                            continue
                        
                        # 如果这是一条设计链（含有New_），且当前原始残基 o 不在用户请求的 contigs 中
                        # 那么这通常是多余的原始背景，应该过滤掉！
                        if is_design_chain and (o not in requested_origins):
                            continue
            ...
            # [MODIFIED] 获取 write_full_chains 配置
            wfc_config = self.design_conf.get('write_full_chains', False)
            
            # 解析允许保留的背景链 ID
            allowed_bg_chains = None
            # 如果配置是列表或元组（即指定了 Chain ID），则转换为集合
            if hasattr(wfc_config, '__iter__') and not isinstance(wfc_config, str):
                allowed_bg_chains = set(list(wfc_config))
            # 如果配置是 True，则允许所有链（allowed_bg_chains 保持为 None）
            elif wfc_config is True:
                allowed_bg_chains = None
            # 如果配置是 False，allowed_bg_chains 保持 None，但这里的逻辑不需要变，
            # 因为 Sampler 那边会根据 bool(wfc_config) 为 False 而跳过整个输出逻辑，
            # 不过为了 Target 内部数据一致性，我们可以在循环里加判断。
            ...
            for ch in orig_chains:
                ...
                # [MODIFIED] 核心过滤逻辑：
                # 1. 如果 wfc_config 是 False，这里其实加不加无所谓，因为 sampler 不会写出。
                # 2. 如果 allowed_bg_chains 不为 None (即指定了列表)，且 ch 不在列表中 -> 跳过
                if allowed_bg_chains is not None:
                    if ch not in allowed_bg_chains:
                        continue
    ...
    def parse_bond_condition(self):
        ...
        #
        # Terminal semantics with N/C clones enabled:
        # - A/start (or A/nter) -> N-terminal clone node
        # - A/end   (or A/cter) -> C-terminal clone node
        # - A/1, A/2, ...       -> body residues (not terminal clone nodes)
        # - A/body_start        -> first body residue
        # - A/body_end          -> last body residue
        ...
        for bond_contig in self.design_conf.bond_condition:
            ...
            def get_indices(spec):
                ...
                def get_res_idx(res_spec):
                    ...
                    head_mask = getattr(self, "full_head_mask", None)
                    tail_mask = getattr(self, "full_tail_mask", None)

                    def is_terminal_clone(idx: int) -> bool:
                        is_head = bool(head_mask[idx].item()) if head_mask is not None and idx < head_mask.shape[0] else False
                        is_tail = bool(tail_mask[idx].item()) if tail_mask is not None and idx < tail_mask.shape[0] else False
                        return is_head or is_tail

                    body_indices = [idx for idx in chain_indices if not is_terminal_clone(idx)]

                    if token in ('start', 'nter'):
                        return min(chain_indices) if chain_indices else -1
                    elif token in ('end', 'cter'):
                        return max(chain_indices) if chain_indices else -1
                    elif token in ('body_start', 'first_body'):
                        return min(body_indices) if body_indices else -1
                    elif token in ('body_end', 'last_body'):
                        return max(body_indices) if body_indices else -1
                    else:
                        res_num = int(token)
                        # Prefer true body residues over terminal clones, so that
                        # A/1 means the first body residue while A/start means the N node.
                        for idx in body_indices:
                            if self.full_pdb_idx[idx] == (chain_id, res_num):
                                return idx
                        # Fallback when there are no terminal clones for this chain.
                        try:
