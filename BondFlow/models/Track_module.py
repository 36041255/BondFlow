...
from rfdiff.util_module import *
from BondFlow.models.Attention_module import *
from rfdiff.SE3_network import SE3TransformerWrapper
...
class MSAPairStr2MSA(nn.Module):
    def __init__(self, d_msa=256, d_pair=128, n_head=8, d_state=16,
                 d_hidden=32, p_drop=0.15, d_condition=None):
        super(MSAPairStr2MSA, self).__init__()
        self.norm_pair = AdaLayerNorm(d_pair,d_condition, gate=False)
        self.proj_pair = nn.Linear(d_pair+36, d_pair)
        self.norm_state = AdaLayerNorm(d_state,d_condition, gate=False)
        ...
        self.ff = Transition(d_msa, 4, d_condition=d_condition, p_drop=p_drop)
    ...
    def forward(self, msa, pair, rbf_feat, state, condtion_1d,condtion_2d,mask=None):
        '''
        Inputs:
        ...
        pair = self.norm_pair(pair,condtion_2d)
        ...
        state= self.norm_state(state,condtion_1d)
        ...
        stats_nan(msa, 'msa_row_attn')
        # msa = msa + self.col_attn(msa,mask=mask)
        # stats_nan(msa, 'msa_col_attn')
        msa = msa + self.ff(msa,condition=condtion_1d.unsqueeze(1))
        return msa

class PairStr2Pair(nn.Module):
    def __init__(self, d_pair=128, n_head=4, d_hidden=32, d_rbf=36, p_drop=0.15,d_condition=None):
        ...
        self.ff = Transition(d_pair, 2, d_condition,p_drop=p_drop)
    ...
    def forward(self, pair, rbf_feat,condition_2d,mask=None):
        B, L = pair.shape[:2]

        rbf_feat = self.proj_rbf(F.relu_(self.emb_rbf(rbf_feat)))

        pair = pair + self.drop_row(self.row_attn(pair, rbf_feat,mask=mask))
        pair = pair + self.drop_col(self.col_attn(pair, rbf_feat,mask=mask))
        pair = pair + self.ff(pair,condition_2d)
        return pair

class MSA2Pair(nn.Module):
    def __init__(self, d_msa=256, d_pair=128, d_hidden=32, p_drop=0.15,d_condition=None):
        super(MSA2Pair, self).__init__()
        self.norm = AdaLayerNorm(d_msa,d_condition, gate=False)
    ...
    def forward(self, msa, pair,condition_1d):
        B, N, L = msa.shape[:3]
        assert N == 1, "Only support single sequence in MSA for now"
        msa = self.norm(msa,condition_1d.unsqueeze(1))
        left = self.proj_left(msa).squeeze(1)
        right = self.proj_right(msa).squeeze(1)
        out = torch.einsum('bli,bmj->blmij', left, right).reshape(B, L, L, -1)
...
class Str2Str(nn.Module):
    def __init__(self, d_msa=256, d_pair=128, d_state=16, d_condition=None,
            SE3_param={'l0_in_features':32, 'l0_out_features':16, 'num_edge_features':32}, p_drop=0.1,sc_pred=False):
        super(Str2Str, self).__init__()
        
        # initial node & pair feature process
        self.norm_msa = AdaLayerNorm(d_msa,d_condition, gate=False)
        self.norm_pair = AdaLayerNorm(d_pair,d_condition, gate=False)
        self.norm_state = AdaLayerNorm(d_state,d_condition, gate=False)
    ...
    def forward(self, msa, pair, R_in, T_in, xyz, state, idx, condition_1d,condition_2d,
                motif_mask=None, top_k=64, mask=None,eps=1e-5):
        B, N, L = msa.shape[:3]
        ...
        node = self.norm_msa(msa[:,0].float(),condition_1d)
        pair = self.norm_pair(pair.float(),condition_2d)
        state = self.norm_state(state.float(),condition_1d)
        node = torch.cat((node, state), dim=-1)
        node = self.embed_x(node)
        pair = self.embed_e1(pair)
        ...
        pair = self.embed_e2(pair)
        # pair = self.norm_edge2(self.embed_e2(pair))

        # define graph
        if top_k != 0:
            G, edge_feats = make_topk_graph(xyz[:,:,1,:], pair, idx, top_k=top_k,mask=mask)
        else:
            G, edge_feats = make_full_graph(xyz[:,:,1,:], pair, idx, top_k=top_k,mask=mask)
...
class IterBlock(nn.Module):
    def __init__(self, d_msa=256, d_pair=128,
        ...
        self.str2str = Str2Str(d_msa=d_msa, d_pair=d_pair,
                               d_state=SE3_param['l0_out_features'],
                               SE3_param=SE3_param,
                               p_drop=p_drop,d_condition=d_condition)

    def forward(self, msa, pair, R_in, T_in, xyz, state, idx, condition_1d, condition_2d,mask=None ,use_checkpoint=False):
        rbf_feat = rbf(torch.cdist(xyz[:,:,1,:], xyz[:,:,1,:]))

        if use_checkpoint:
            msa = checkpoint.checkpoint(create_custom_forward(self.msa2msa, mask=mask), msa, pair, rbf_feat, state, condition_1d, condition_2d)
            pair = checkpoint.checkpoint(create_custom_forward(self.msa2pair), msa, pair, condition_1d)
            pair = checkpoint.checkpoint(create_custom_forward(self.pair2pair, mask=mask), pair, rbf_feat, condition_2d)
            R, T, state, alpha = checkpoint.checkpoint(create_custom_forward(self.str2str, top_k=0, mask=mask), msa, pair, R_in, T_in, xyz, state, idx, condition_1d, condition_2d)
        else:
            msa = self.msa2msa(msa, pair, rbf_feat, state,condition_1d, condition_2d,mask=mask)
            stats_nan(msa, 'msa2msa')
            pair = self.msa2pair(msa, pair, condition_1d)
            stats_nan(pair, 'msa2pair')
            pair = self.pair2pair(pair, rbf_feat, condition_2d, mask=mask)
            stats_nan(pair, 'pair2pair')
            R, T, state, alpha = self.str2str(msa, pair, R_in, T_in, xyz, state, idx, condition_1d, condition_2d, mask=mask,top_k=0) 
            stats_nan(R, 'R_str2str')
            stats_nan(T, 'T_str2str')

        stats_nan(msa, 'msa_latent_block')
        stats_nan(pair, 'pair_block')
        stats_nan(R, 'R_block')
        stats_nan(T, 'T_block')
        stats_nan(state, 'state_block')

        return msa, pair, R, T, state, alpha

class IterativeSimulator(nn.Module):
    def __init__(self, n_main_block=12, n_ref_block=4,
                 d_msa=256, d_pair=128, d_hidden=32,
                 n_head_msa=8, n_head_pair=4,d_condition=128,
        ...
        if n_ref_block > 0:
            self.str_refiner = Str2Str(d_msa=d_msa, d_pair=d_pair,
                                       ...
                                       SE3_param=SE3_param_topk,d_condition=d_condition,
                                       p_drop=p_drop)
    ...
    def forward(self, seq, msa, pair, xyz_in, state, idx, 
                condition_1d, condition_2d,mask=None,use_checkpoint=False):
        """
        input:
        ...
        for i_m in range(self.n_main_block):
            ...
            print_gpu_memory(step_name=f"Main Block {i_m+1}/{self.n_main_block}")
        ...
        valid_alpha = [t for t in alpha_s if t is not None]
    
        if len(valid_alpha) == 0:  # 如果所有都是None
            alpha_s = []
        else:
            alpha_s = torch.stack(valid_alpha, dim=0)
