...
class MSAPairStr2MSA(nn.Module):
    ...
    def forward(self, msa, pair, rbf_feat, state, mask=None):
        '''
        Inputs:
...
class PairStr2Pair(nn.Module):
    ...
    def forward(self, pair, rbf_feat,mask=None):
        B, L = pair.shape[:2]
...
class Str2Str(nn.Module):
    ...
    def forward(self, msa, pair, R_in, T_in, xyz, state, idx, motif_mask, top_k=64, mask=None,eps=1e-5):
        B, N, L = msa.shape[:3]
        ...
        node = self.norm_msa(msa[:,0].float())
        pair = self.norm_pair(pair.float())
        state = self.norm_state(state.float())
...
class IterBlock(nn.Module):
    def __init__(self, d_msa=256, d_pair=128,
                 ...
                 d_hidden=32, d_hidden_msa=None, d_time=256, p_drop=0.15,
        ...
        self.str2str = Str2Str(d_msa=d_msa, d_pair=d_pair,
                               ...
                               SE3_param=SE3_param,
                               p_drop=p_drop)
        self.adaLN_msa = AdaLayerNorm(d_msa,d_time)
        self.adaLN_pair = AdaLayerNorm(d_pair, d_time)
        self.adaLN_state = AdaLayerNorm(SE3_param['l0_out_features'], d_time)


    def forward(self, msa, pair, R_in, T_in, xyz, state, idx, t_emb,motif_mask,mask=None ,use_checkpoint=False):
        rbf_feat = rbf(torch.cdist(xyz[:,:,1,:], xyz[:,:,1,:]))
        # msa = self.adaLN_msa(msa,t_emb)
        # pair = self.adaLN_pair(pair,t_emb)
        # state = self.adaLN_state(state,t_emb)
        if use_checkpoint:
            ...
            msa = self.msa2msa(msa, pair, rbf_feat, state,mask=mask)
            stats_nan(msa, 'msa2msa')
            pair = self.msa2pair(msa, pair)
            stats_nan(pair, 'msa2pair')
            pair = self.pair2pair(pair, rbf_feat, mask=mask)
            stats_nan(pair, 'pair2pair')
            R, T, state, alpha = self.str2str(msa, pair, R_in, T_in, xyz, state, idx, motif_mask=motif_mask, mask=mask,top_k=0) 
...
class IterativeSimulator(nn.Module):
    ...
    def forward(self, seq, msa, msa_full, pair, xyz_in, state, idx, time_emb=None,mask=None,use_checkpoint=False, motif_mask=None):
        """
        input:
