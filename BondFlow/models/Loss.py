...
def compute_terminal_body_maps(
    ...
    If nc_anchor is provided (B, L, L, 2), it takes precedence:
      - head_to_body is derived from nc_anchor[..., 0]
      - tail_to_body is derived from nc_anchor[..., 1]
    ...
    # 1. Try to use nc_anchor if available
    if nc_anchor is not None:
        # nc_anchor: [B, L, L, 2]
        # Layer 0: N-terminal anchors. Row i has a 1 at column j if head i anchors to body j.
        # Layer 1: C-terminal anchors. Row i has a 1 at column j if tail i anchors to body j.
        
        # Head mapping
        # We only care about rows where head_mask is True
        # For robustness, we take argmax along dim 1 (columns) for each row.
        # We need to mask out rows that are not heads to keep them as -1.
        
        # [B, L, L]
        n_anchor_mat = nc_anchor[..., 0]
        # Find indices of max value (should be 1). 
        # Note: if a row is all zeros (no anchor), argmax returns 0. We need to handle this.
        max_vals_n, body_indices_n = n_anchor_mat.max(dim=2) # [B, L], [B, L]
        
        # Valid heads are those where head_mask is True AND we actually found an anchor (max > 0.5)
        valid_heads = (head_mask.bool()) & (max_vals_n > 0.5)
        head_to_body[valid_heads] = body_indices_n[valid_heads]

        # Tail mapping
        c_anchor_mat = nc_anchor[..., 1]
        max_vals_c, body_indices_c = c_anchor_mat.max(dim=2)
        valid_tails = (tail_mask.bool()) & (max_vals_c > 0.5)
        tail_to_body[valid_tails] = body_indices_c[valid_tails]
        
        return head_to_body, tail_to_body

    # 2. Fallback to heuristic (original logic)
...
class BondCoherenceLoss(nn.Module):
    ...
    def _get_termini_masks(
        ...
        return infer_termini_masks(res_mask, head_mask, tail_mask)
    ...
    def forward(self,
        ...
        if not silent: #and False:
            ...
            for b_i in range(B):
                ...
                t_idx = torch.where(tail_mask[b_i])[0]
                if len(h_idx) > 0 or len(t_idx) > 0:
                    info_strs = []
                    for h in h_idx:
                        info_strs.append(f"H{h.item()}->{h2b_debug[b_i, h].item()}")
                    for t in t_idx:
        ...
        if all_atom_coords is not None and aatype is not None:
            ...
            if self.lambda_energy_bce > 0:
                ...
                if energy_mask.any():
                    ...
                    # --- Compatibility-based weighting using compat_matrix & current sequence ---
                    # Build pairwise AA compatibility weights [B, L, L] from per-residue distributions.
                    # This uses the same compat_matrix as seq_term, but only affects the geometry–probability
                    # consistency term, so different residue pairs contribute with different strengths.
                    pair_weight = None
                    try:
                        # Use aatype (ground-truth or provided residue types) to build compatibility weights.
                        # This avoids dependence on noisy seq_logits and keeps the geometry-probability
                        # consistency term purely conditioned on residue identities.
                        if (aatype is not None) and hasattr(self, "compat_matrix") and (self.compat_matrix is not None):
                            Kc = int(self.compat_matrix.shape[0])
                            aa_safe = aatype.long().clamp(0, Kc - 1)  # [B, L]
                            probs = F.one_hot(aa_safe, num_classes=Kc).float()  # [B, L, Kc]

                            # pair_weight[b,i,j] ~ compatibility of (aa_i, aa_j)
                            Cp_pw = torch.matmul(probs, self.compat_matrix)                 # [B, L, Kc]
                            pair_weight = torch.matmul(Cp_pw, probs.transpose(-1, -2))      # [B, L, L]
                            pair_weight = pair_weight.clamp_min(self.eps)
                        else:
                            pair_weight = torch.ones_like(bond_matrix, dtype=weights.dtype, device=weights.device)
                    except Exception:
                        # On any failure, fall back to uniform weights
                        pair_weight = torch.ones_like(bond_matrix, dtype=weights.dtype, device=weights.device)

                    # Apply compatibility weights (detached so this term does not backprop into sequence)
                    weights = weights * pair_weight.detach()
...
class OpenFoldClashLoss(nn.Module):
    ...
    def __init__(self,
        ...
        """
        Args:
            reduction: 'mean' (default) - divide by denom (mean over violating pairs);
                       'sum' - return raw sum (for classifier guidance sampling).
        """
        super().__init__()
        self.reduction = str(reduction).lower()
        assert self.reduction in ('mean', 'sum'), f"reduction must be 'mean' or 'sum', got {reduction}"
        self.link_csv_path = link_csv_path
        self.bond_threshold = float(bond_threshold)
    ...
    def _gather_link_masks(self, seq_pred: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Vectorized gather over batch: build (B,L,L) AA index grids and index [K,K,14,14]
        aa_i = seq_safe.unsqueeze(2)  # [B, L, 1]
        aa_j = seq_safe.unsqueeze(1)  # [B, 1, L]
        allowed = allowed_buf[aa_i, aa_j]  # [B, L, L, 14, 14]
        ref = ref_buf[aa_i, aa_j]         # [B, L, L, 14, 14]
        return allowed, ref
    ...
    def forward(self,
        ...
        if bond_mat is not None and (self.link_allowed_mask.any() or self.treat_adjacent_as_bonded):
            ...
            # termini-gated N/C atoms (vectorized using precomputed aa_isN_mask / aa_isC_mask)
            Kmask = self.aa_atom_exists.shape[0]
            seq_safe_mask = seq_pred.clamp(min=0, max=Kmask - 1)
            # Use precomputed per-AA atom type masks to avoid per-batch Python loops
            isN = self.aa_isN_mask[seq_safe_mask]  # [B, L, 14]
            isC = self.aa_isC_mask[seq_safe_mask]  # [B, L, 14]
            ...
            head_to_body_idx, tail_to_body_idx = compute_terminal_body_maps(res_mask, head_mask, tail_mask, nc_anchor)
            ...
            if self.log_raw:
                for b_i in range(B):
                    ...
                    if len(h_idx) > 0 or len(t_idx) > 0:
                        info_strs = []
                        for h in h_idx:
                            info_strs.append(f"H{h.item()}->{head_to_body_idx[b_i, h].item()}")
                        for t in t_idx:
                            info_strs.append(f"T{t.item()}->{tail_to_body_idx[b_i, t].item()}")
                        print(f"[OpenFoldClashLoss] b={b_i} TerminiMap: {', '.join(info_strs)}")
