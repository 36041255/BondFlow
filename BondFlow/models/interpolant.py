...
class Interpolant:
    ...
    def __init__(self, cfg,device="cpu"):
        ...
        self._ss_cfg = cfg.get('ss', {}) # Add ss config
        ...
        self._ss_dsm_proj = DSMProjection()
    ...
    def igso3(self):
        if self._igso3 is None:
            ...
            self._igso3 = so3_utils.SampleIGSO3(
                1000, sigma_grid, cache_dir=self._cache_dir)
        return self._igso3       
    ...
    def _corrupt_rotmats(self, rotmats_1, t, res_mask, diffuse_mask):
        ...
        rotmats_t = (
            ...
            + identity[None, None] * (1 - res_mask[..., None, None].float())
        )
    ...
    def _corrupt_aatypes(self, aatypes_1, t, res_mask, diffuse_mask):
        ...
        if self._aatypes_cfg.interpolant_type == "masking":
            ...
            aatypes_t = aatypes_t * res_mask.float() + du.MASK_TOKEN_INDEX * (1 - res_mask.float())
    ...
    def _is_doubly_stochastic_symmetric(self, matrix, tol=1e-5):
        """
        检验矩阵是否为对称双随机矩阵。
        
        Args:
            matrix: 形状为 (B, L, L) 的矩阵
            tol: 容差，用于浮点数比较
            
        Returns:
            bool: 如果所有批次都满足条件返回 True，否则返回 False
        """
        B = matrix.shape[0]
        
        # 检查非负性
        if torch.any(matrix < -tol):
            return False
        
        # 检查对称性：矩阵应该等于其转置
        diff_sym = torch.abs(matrix - matrix.transpose(1, 2))
        if torch.any(diff_sym > tol):
            return False
        
        # 检查双随机性：每行和每列的和应该接近1
        row_sums = matrix.sum(dim=-1)  # (B, L)
        col_sums = matrix.sum(dim=-2)  # (B, L)
        
        row_diff = torch.abs(row_sums - 1.0)
        col_diff = torch.abs(col_sums - 1.0)
        
        if torch.any(row_diff > tol) or torch.any(col_diff > tol):
            return False
        
        return True

    def _sample_ss_prior(self, res_mask):
        """
        使用高斯随机 logits + Sinkhorn 投影，生成对称的双随机 ss 先验。
        仅在 res_mask 指定的有效残基子块上进行归一化。
        """
        B, L = res_mask.shape[:2]
        res_mask_2d = res_mask[:, :, None] * res_mask[:, None, :]
        mask_2d = res_mask_2d.bool()

        # 随机部分：高斯 logits -> Sinkhorn 双随机矩阵
        # 添加重试机制，确保生成的矩阵满足对称双随机条件
        max_retries = 10
        ss_rand = None
        
        for retry in range(max_retries):
            ss_logits = torch.randn(B, L, L, device=self._device)
            ss_logits = 0.5 * (ss_logits + ss_logits.transpose(1, 2))
            # ss_logits = ss_logits * res_mask_2d

            ss_rand = self._ss_dsm_proj(ss_logits)
            
            # 检验是否满足对称双随机矩阵条件
            if self._is_doubly_stochastic_symmetric(ss_rand):
                break
        
        # 如果经过 max_retries 次尝试仍未满足条件，使用最后一次的结果
        # （在实际应用中，DSMProjection 应该总是生成有效的双随机矩阵）
      
        # 单位阵部分：在有效残基子块上保留对角
        eye = torch.eye(L, device=self._device).unsqueeze(0).expand(B, -1, -1)
        #eye = eye * res_mask_2d

        # 配置中的 prior_mix_lambda 控制随机先验和单位阵先验的混合比例
        prior_mix_lambda = getattr(self._ss_cfg, "prior_mix_lambda", 1.0)
        prior_mix_lambda = float(prior_mix_lambda)
        prior_mix_lambda = max(0.0, min(1.0, prior_mix_lambda))

        ss_0 = (1.0 - prior_mix_lambda) * eye + prior_mix_lambda * ss_rand
        return ss_0

    def _corrupt_ss(self, ss_1, t, res_mask, diffuse_mask):
        """Corrupts doubly_stochastic matrix."""
        # Residue 掩码二维版本（仅在有效残基子块上操作）
        res_mask_2d = res_mask[:, :, None] * res_mask[:, None, :]
        # 使用统一的先验采样函数
        ss_0 = self._sample_ss_prior(res_mask=res_mask)

        if self._ss_cfg.train_schedule == 'linear':
            ss_t = (1 - t[..., None]) * ss_0 + t[..., None] * ss_1
        else:
            raise ValueError(f'Unknown ss schedule {self._ss_cfg.train_schedule}')

        # Masking is applied to rows and columns.
        ss_t = ss_t * res_mask_2d
        # Apply diffuse mask
        ss_t = ss_t * diffuse_mask + ss_1 * (1 - diffuse_mask)    
        ss_t = smu.make_sub_doubly2doubly_stochastic(ss_t)
        return ss_t
    ...
    def corrupt_batch(
        ...
        ss_1,
        res_mask=None,
        trans_diffuse_mask=None,
        rots_diffuse_mask=None,
        aatypes_diffuse_mask=None,
        ss_diffuse_mask=None,
        t=None,
    ):
        """Corrupts a batch of data.

        Args:
            trans_1: Ground truth translations. Shape (B, N, 3).
            rotmats_1: Ground truth rotations. Shape (B, N, 3, 3).
            aatypes_1: Ground truth amino acid types. Shape (B, N).
            ss_1: Ground truth secondary structure. Shape (B, N, N).
            res_mask: Residue mask. Shape (B, N).
            trans_diffuse_mask: Translation-specific diffuse mask. Shape (B, N).
            rots_diffuse_mask: Rotation-specific diffuse mask. Shape (B, N).
            aatypes_diffuse_mask: Amino acid type-specific diffuse mask. Shape (B, N).
            ss_diffuse_mask: Secondary structure-specific diffuse mask. Shape (B, N, N).
        """
        
        # Modality-specific diffuse masks
        if res_mask is None:
            res_mask = torch.ones(trans_1.shape[:2],device=self._device,dtype=torch.float)

        if trans_diffuse_mask is None:
            trans_diffuse_mask = res_mask
        if rots_diffuse_mask is None:
            rots_diffuse_mask = res_mask
        if aatypes_diffuse_mask is None:
            aatypes_diffuse_mask = res_mask
        if ss_diffuse_mask is None:
            ss_diffuse_mask = res_mask[:, :, None] * res_mask[:, None, :]
        ...
        if t is None:
            ...
            assert t.shape == (num_batch,), f'Invalid t shape: {t.shape}'
            t = t[:, None]  # Ensure t is of shape (B, 1)
            so3_t = t
            r3_t = t
            cat_t = t
            ss_t = t
        ...
        if self._trans_cfg.corrupt:
            trans_t = self._corrupt_trans(
                trans_1, r3_t, res_mask, trans_diffuse_mask)
        else:
            trans_t = trans_1
        ...
        if self._rots_cfg.corrupt:
            rotmats_t = self._corrupt_rotmats(rotmats_1, so3_t, res_mask, rots_diffuse_mask)
        else:
            rotmats_t = rotmats_1
        ...
        if self._aatypes_cfg.corrupt:
            aatypes_t = self._corrupt_aatypes(aatypes_1, cat_t, res_mask, aatypes_diffuse_mask)
        else:
            aatypes_t = aatypes_1
        
        if self._ss_cfg.get('corrupt'):
            # For ss, we can use r3_t, so3_t or cat_t. Let's use r3_t for now.
            ss_t = self._corrupt_ss(ss_1, ss_t, res_mask, ss_diffuse_mask)
        else:
            ss_t = ss_1
        ...
        noisy_batch = {
            ...
            # 'ss_diffuse_mask': ss_diffuse_mask,
            # 'so3_t': so3_t,
            # 'r3_t': r3_t,
            # 'cat_t': cat_t,
            'trans_t': trans_t,
    ...
    def _aatypes_euler_step(self, d_t, t, logits_1, aatypes_t):
        ...
        if self._aatypes_cfg.interpolant_type == "masking":
            ...
            step_probs = d_t * pt_x1_probs * ((1+ self._aatypes_cfg.noise*t) / ((1 - t) + 1e-8)) # (B, D, S)
            ...
            step_probs = d_t * (pt_x1_probs * ((1 + N + N * (S - 1) * t) / ((1-t) + 1e-8)) + N * pt_x1_eq_xt_prob )
    ...
    def _aatypes_euler_step_purity(self, d_t, t, logits_1, aatypes_t):
        ...
        unmask_probs = (d_t * ( (1 + self._aatypes_cfg.noise * t) / ((1-t) + 1e-8)).to(device)).clamp(max=1) # scalar
    ...
    def _ss_euler_step(self, d_t, t, ss_1, ss_t):
        """Euler step for sub-stochastic matrix."""
        if self._ss_cfg.sample_schedule == 'linear':
            ss_vf = (ss_1 - ss_t) / (1 - t)
        else:
            raise ValueError(f'Unknown ss sample schedule {self._ss_cfg.sample_schedule}')
        return ss_t + ss_vf * d_t
    ...
    def sample_step(
        ...
        t_1,
        t_2,
        trans_t_1, rotmats_t_1, aatypes_t_1, ss_t_1,
        trans_diffuse_mask, rots_diffuse_mask, aatypes_diffuse_mask, ss_diffuse_mask,
        batch = None,
        ...
        pred_ss_1 = model_out.get('pred_ss')

        num_batch, num_res = trans_t_1.shape[:2]
        device = trans_t_1.device

        # Setup ground truth
        if trans_1 is None:
            trans_1 = torch.zeros(num_batch, num_res, 3, device=device)
        if rotmats_1 is None:
            rotmats_1 = torch.eye(3, device=device)[None, None].repeat(num_batch, num_res, 1, 1)
        if aatypes_1 is None:
            aatypes_1 = torch.zeros((num_batch, num_res), device=device).long()
        if ss_1 is None and self._ss_cfg.get('corrupt'):
            ss_1 = torch.zeros(num_batch, num_res, num_res, device=device)
        ...
        if self._cfg.sampling.self_condition and batch is not None:
            batch['trans_sc'] = _trans_diffuse_mask(
                pred_trans_1, trans_1, trans_diffuse_mask)
            if forward_folding:
                batch['aatypes_sc'] = logits_1
            else:
                batch['aatypes_sc'] = _aatypes_diffuse_mask(
                    pred_logits_1, logits_1, aatypes_diffuse_mask)
        ...
        ss_t_2 = None
        if self._ss_cfg.get('corrupt') and pred_ss_1 is not None and ss_t_1 is not None:
            ss_t_2 = self._ss_euler_step(d_t, t_1, pred_ss_1, ss_t_1)

        # if trans_1 is not None:
        #     trans_t_2 = _trans_diffuse_mask(trans_t_2, trans_1, trans_diffuse_mask)
        # if rotmats_1 is not None:
        #     rotmats_t_2 = _rots_diffuse_mask(rotmats_t_2, rotmats_1, rots_diffuse_mask)
        # if aatypes_1 is not None:
        #     aatypes_t_2 = _aatypes_diffuse_mask(aatypes_t_2, aatypes_1, aatypes_diffuse_mask)
        # if ss_t_2 is not None and ss_1 is not None:
        #     diffuse_mask_2d = ss_diffuse_mask
        #     ss_t_2 = ss_t_2 * diffuse_mask_2d + ss_1 * (1 - diffuse_mask_2d)

        return trans_t_2, rotmats_t_2, aatypes_t_2, ss_t_2, (pred_trans_1, pred_rotmats_1, pred_aatypes_1)
    ...
    def sample(
        ...
        ss_0 = None
        if self._ss_cfg.get('corrupt'):
            # 与 _corrupt_ss 保持一致，使用统一的先验采样函数
            ss_0 = self._sample_ss_prior(res_mask=res_mask)
        ...
        trans_diffuse_mask = batch.get('trans_diffuse_mask', diffuse_mask)
        rots_diffuse_mask = batch.get('rots_diffuse_mask', diffuse_mask)
        aatypes_diffuse_mask = batch.get('aatypes_diffuse_mask', diffuse_mask)
        ss_diffuse_mask = batch.get('ss_diffuse_mask', diffuse_mask)
        ...
        trans_t_1, rotmats_t_1, aatypes_t_1, ss_t_1 = trans_0, rotmats_0, aatypes_0, ss_0
        ...
        for t_2 in ts[1:]:
            ...
            # Process model output and take step
            trans_t_2, rotmats_t_2, aatypes_t_2, ss_t_2, clean_preds = self.sample_step(
                model_out, t_1, t_2,
                trans_t_1, rotmats_t_1, aatypes_t_1, ss_t_1,
                trans_1, rotmats_1, aatypes_1, ss_1,
                trans_diffuse_mask, rots_diffuse_mask, aatypes_diffuse_mask, ss_diffuse_mask,
                batch, forward_folding, inverse_folding
            )
            pred_trans_1, pred_rotmats_1, pred_aatypes_1 = clean_preds
            clean_traj.append((frames_to_atom37(pred_trans_1, pred_rotmats_1), pred_aatypes_1.detach().cpu()))

            trans_t_1, rotmats_t_1, aatypes_t_1, ss_t_1 = trans_t_2, rotmats_t_2, aatypes_t_2, ss_t_2
