...
- 因此 `A|A` 和 `A|A:0:FIX` 不同：前者是“保留原值并锁住”，后者是“显式写成 0 并锁住”
- 多条 `bond_condition` 按书写顺序依次应用；如果有重叠区域，后面的显式赋值会覆盖前面的结果
...
- name: type_soft_bond_count
  ...
  region: null                         # 可选：限制 guidance 作用的 2D 区域
  reference_space: design             # design -> full_pdb_idx; origin -> full_origin_pdb_idx
  region_mode: include                # include / exclude
  types:                               # 键类型配置列表
    - name: disulfide                  # 二硫键
...
- `region` 支持 bond_condition 风格的 2D 区域字符串；默认按 design 空间解析
- 当你想直接引用原始 PDB 位点时，可设 `reference_space: origin`
...
**示例**：只在某个原始 PDB 锚点附近统计/引导 covalent 键
...
#### 6. ResidueBondGuidance

对某个锚点残基的成键行为做采样期引导，直接修改 `ss_t_2`：
...
说明：
- `mode: unique_in_candidates` 会让 anchor 在候选集合里只保留 1 个 partner。
- `candidates` 不写时，默认会在所有 `bond_condition` 允许的位置里选。
- `top_k` 如果大于 0，会先按当前 anchor 行的 bond 分数只保留前 K 个候选，再在其中做引导。这对“只想要少数几个候选位点参与竞争、避免很多位置都被轮流拉成兼容残基”特别有用。
- `unique_in_candidates` 内部已经固定成更稳的“只奖励当前 strongest partner、不给长尾求和奖励”的更新形式，不需要额外再配 `loss_form` 一类参数。
- 实现上会把当前最强候选当作主峰拉高，同时软压其它候选边和候选外边；相比早先的硬 one-hot 覆盖，这种形式通常更不容易把很多位置都间接推成兼容残基。
